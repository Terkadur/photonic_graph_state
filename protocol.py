...
def protocol_to_circuit(protocol: list[tuple]) -> QuantumCircuit:
    '''
    Converts a photonic generation protocol into a Qiskit QuantumCircuit

    :param protocol: A protocol describing the generation of a photonic state
    :type protocol: list[tuple]

    :return: A Qiskit QuantumCircuit describing the generation of a photonic state
...
def protocol_to_state(protocol: list[tuple], progress: bool = False, check_measurements: bool = True) -> Expr:
    '''
    Converts a photonic generation protocol into a SymPy statevector

    :param protocol: A protocol describing the generation of a photonic state
    :type protocol: list[tuple]

    :param progress: A boolean option to continuously print out the progress of calculating the state (useful for many qubits), defaults to False
    :type progress: bool

    :param check_measurements: A boolean option to check that the statevector is independent of measurement outcomes, defaults to True
    :type check_measurements: bool

    :return: A SymPy statevector representing the photonic state generated by protocol
    :rtype: Expr
    '''
    n_qubits = 0
    for step in protocol:
...
def photon_permuter(graph: Graph, progress: bool = False) -> tuple[Graph, int]:
    '''
    Takes in a photonic graph and permutes the labels to find a labelling with the minimum number of required emitters

    :param graph: A graph describing a photonic graph state for which we want to minimize the number of required emitters
    :type graph: Graph

    :param progress: A boolean option to continuously print out the progress of checking permutations (useful for many qubits), defaults to False
    :type progress: bool

    :return: A tuple of an equivalent graph with the optimal labelling and the associated number of emitters
    ...
    return Graph(best_edges), min_emitters
